...
from ui.squatting_form import Ui_Frame as ui_form
...
def squat(human):
    ...
    if len(pnts3) == 3:
        ...
        prev_squat_pos = 1
        if angle <= 90 and angle3 >= 65:
            squat_pos = 0
        elif 90 <= angle <= 150 and angle3 >= 65:
            squat_pos = 2
        elif angle <= 90 and angle3 <= 65:
            squat_pos = 3
        elif 150 < angle and angle3 >= 65:
            squat_pos = 4
        else:
            squat_pos = 1
...
class squatting_form(ui_form, QFrame):
    def __init__(self):
        super(squatting_form, self).__init__()
        self.setupUi(self)
        icon = QtGui.QIcon()
            ...
            while not userWantsToExit:
                ...
                if opWrapper.waitAndPop(datumProcessed):
                    if not args[0].no_display:
                        ...
                        count_of_wrong_string = str(count_of_wrong)
                        if count_of_wrong > 0:
                            count_of_wrong_string = count_of_wrong_string + "\n" + "your waist is not straight"
                        self.right_label.setText(count_string)
                        self.wrong_label.setText(count_of_wrong_string)
                else:
                    self.camera_label.clear()
                    break
            self.camera_label.setPixmap(QPixmap(""))
        else:
            self.wrong_label.setText("not have \n number")

    def close_event(self, event):
        logger.info("Close squatting window")
        global userWantsToExit
        userWantsToExit = True
        self.close()
